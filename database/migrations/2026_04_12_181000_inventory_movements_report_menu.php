...
{
    /**
     * Posted inventory movements register (quantity subledger read) under Inventory → Reports + user_rights.
     */
    public function up(): void
    {
        ...
        $route = '/inventory/reports/inventory-movements';
        ...
        if ($existing) {
            DB::table('menus')->where('id', $existing->id)->update([
                ...
                'menu_name' => 'Posted inventory movements',
                'icon' => 'history',
                'status' => 1,
                'sort_order' => 9,
                'updated_at' => now(),
    ...
    public function down(): void
    {
        ...
        $menu = DB::table('menus')
            ->where('module_id', $inventory->id)
            ->where('route', '/inventory/reports/inventory-movements')
            ->first();
