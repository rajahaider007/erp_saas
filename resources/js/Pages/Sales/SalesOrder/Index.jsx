...
export default function SalesOrderIndex({ orders, filters = {} }) {
  ...
  const handleDelete = (id) => {
    if (confirm('آپ یقینی ہیں کہ اس order کو حذف کریں گے؟')) {
  ...
  const getStateColor = (state) => {
    const colors = {
      ...
      in_delivery: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
      ...
      closed: 'bg-gray-400 text-gray-900 dark:bg-gray-600 dark:text-gray-100',
    };
      ...
      <div className="mx-auto max-w-7xl text-gray-900 dark:text-gray-100">
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Order یا Customer کی تلاش کریں"
          ...
          <div>
            ...
            >
              ...
              <option value="in_delivery">ڈیلیوری میں</option>
              <option value="delivered">ڈیلیور شدہ</option>
              <option value="invoiced">بل بھجوایا</option>
              <option value="closed">بند</option>
            </select>
          </div>
        </div>
        ...
        {orders.data && orders.data.length > 0 ? (
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-600">
              <table className="w-full">
                <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-900/50">
                  <tr>
                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Order No</th>
                    ...
                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Deadline</th>
                    <th className="px-6 py-3 text-center text-sm font-semibold text-gray-700 dark:text-gray-300">اعمال</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                  {orders.data.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30">
                      <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{order.sales_order_no}</td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{order.customer?.customer_name}</td>
                      <td className="px-6 py-4 text-right text-sm font-medium text-gray-900 dark:text-white">
                        {parseFloat(order.amount_total).toLocaleString('en-US', {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-6 py-4 text-sm">
                        <span className={`rounded-full px-3 py-1 text-xs font-semibold ${getStateColor(order.state)}`}>{order.state}</span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                        {order.commitment_date ? new Date(order.commitment_date).toLocaleDateString('ur-PK') : '-'}
                      </td>
          ...
          <div className="py-12 text-center">
            <p className="text-lg text-gray-500 dark:text-gray-400">کوئی order نہیں ملا</p>
            <Link
              href={route('sales.order.create')}
