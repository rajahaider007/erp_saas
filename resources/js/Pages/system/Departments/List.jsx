...
const List = () => {
  ...
  const tl = (key, rep = {}) => t(`system.departments.list.${key}`, rep);
  const td = (key, rep = {}) => t(`common.data_table.${key}`, rep);
  const tf = (key, rep = {}) => t(`common.flash.${key}`, rep);
  const { departments: paginated, companies, locations, flash, filters, pageTitle } = usePage().props;
  ...
  const handleDelete = (department) => {
    CustomAlert.fire({
      ...
      text: td('confirm_delete_text', { name: department.department_name }),
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: td('confirm_delete_ok'),
      cancelButtonText: t('common.actions.cancel'),
      onConfirm: () => router.delete(`/system/departments/${department.id}`),
    });
  };
  ...
  const handleBulkDelete = () => {
    ...
    CustomAlert.fire({
      ...
      cancelButtonText: t('common.actions.cancel'),
      onConfirm: () => router.post('/system/departments/bulk-destroy', { ids: selected }, { onSuccess: () => setSelected([]) }),
    });
  };
  const handleBulkStatusChange = (status) => {
    ...
    if (status) {
      CustomAlert.fire({
        ...
        cancelButtonText: t('common.actions.cancel'),
        onConfirm: () => router.post('/system/departments/bulk-status', { ids: selected, status }, { onSuccess: () => setSelected([]) }),
      });
    } else {
      CustomAlert.fire({
        ...
        <div className="manager-header">
          <div className="header-main">
            <div className="title-section">
              <h1 className="page-title"><Users className="title-icon" />{pageTitle || tl('page_title_fallback')}</h1>
              <div className="stats-summary">
                <div className="stat-item"><span>{td('stat_total', { count: paginated?.total || 0 })}</span></div>
                <div className="stat-item"><span>{td('stat_active', { count: paginated?.data?.filter(d=>d.status).length || 0 })}</span></div>
                <div className="stat-item"><span>{td('stat_inactive', { count: paginated?.data?.filter(d=>!d.status).length || 0 })}</span></div>
              </div>
            </div>
            <div className="header-actions">
              <button className="btn btn-icon" onClick={()=>window.location.reload()} title={tl('refresh')}><RefreshCcw size={20} /></button>
              <a href='/system/departments/create' className="btn btn-primary"><Plus size={20} />{tl('add_department')}</a>
            </div>
          </div>
              ...
              <div className="search-section">
                <div className="search-input-wrapper">
                  ...
                  <input
                    ...
                    placeholder={tl('search_departments')}
                    value={search}
                ...
                <div className="filter-group">
                  <label className="filter-label">{tl('location')}</label>
                  <select
                    className="filter-select"
                    value={locationId}
                    onChange={(e) => setLocationId(e.target.value)}
                  >
                    <option value="">{tl('all_locations')}</option>
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th className="sortable" onClick={()=>{}}><div className="th-content">{tl('department')}<ArrowUpDown size={14} className="sort-icon" /></div></th>
                  ...
                  <th className="sortable" onClick={()=>{}}><div className="th-content">{tl('manager')}<ArrowUpDown size={14} className="sort-icon" /></div></th>
              ...
              <tbody>
                {paginated.data.map(department => (
                  <tr key={department.id} className="table-row">
                    ...
                    <td><span className={`status-badge status-${department.status ? 'active' : 'inactive'}`}>{department.status ? t('common.status.active') : t('common.status.inactive')}</span></td>
                    <td><div className="date-cell"><Clock size={14} /><span>{formatDateTime(department.updated_at)}</span></div></td>
                    <td>
                      <div className="actions-cell">
                        <button className="action-btn view" title={tl('view_details')} onClick={()=>router.get(`/system/departments/${department.id}/edit`)}>
                          <Eye size={16} />
                        </button>
                        <button className="action-btn edit" title={tl('edit_department')} onClick={()=>router.get(`/system/departments/${department.id}/edit`)}>
                          <Edit3 size={16} />
                        </button>
                        <button className="action-btn copy" title={tl('duplicate')} onClick={()=>router.get('/system/departments/create', { duplicate: department.id })}>
                          <Copy size={16} />
                        </button>
                        <button className="action-btn delete" title={tl('delete_department')} onClick={()=>handleDelete(department)}>
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
