    ...
    <App>
      <div className="advanced-module-manager">
        <div className="manager-header">
          ...
          </div><button className="btn btn-sm btn-secondary" onClick={()=>setShowColumnSelector(false)}>{t('inventory.item_class_coding.list.close')}</button></div></div>)}
        </div>
        {selected.length>0 && (<div className="bulk-actions-bar"><div className="selection-info"><CheckCircle2 size={20} /><span>{selected.length} selected</span></div><div className="bulk-actions"><button className="btn btn-sm btn-secondary" onClick={()=>setSelected([])}><X size={16} />{t('inventory.item_class_coding.list.clear')}</button><button className="btn btn-sm btn-danger" onClick={handleBulkDelete}><Trash2 size={16} />{t('inventory.item_class_coding.list.delete')}</button></div></div>)}
        <div className="data-table-container"><div className="table-wrapper"><table className="data-table"><thead><tr>
          ...
          {visibleColumns.id && (<th className="sortable" onClick={()=>handleSort('id')}><div className="th-content">{t('inventory.item_class_coding.list.id')}<ArrowUpDown size={14} className={`sort-icon ${sortConfig.key==='id'?'active':''}`} /></div></th>)}
          {visibleColumns.classCode && (<th className="sortable" onClick={()=>handleSort('class_code')}><div className="th-content">{t('inventory.item_class_coding.list.class_code')}<ArrowUpDown size={14} className={`sort-icon ${sortConfig.key==='class_code'?'active':''}`} /></div></th>)}
          {visibleColumns.className && (<th className="sortable" onClick={()=>handleSort('class_name')}><div className="th-content">{t('inventory.item_class_coding.list.class_name')}<ArrowUpDown size={14} className={`sort-icon ${sortConfig.key==='class_name'?'active':''}`} /></div></th>)}
          {visibleColumns.status && (<th className="sortable" onClick={()=>handleSort('is_active')}><div className="th-content">{t('inventory.item_class_coding.list.status')}<ArrowUpDown size={14} className={`sort-icon ${sortConfig.key==='is_active'?'active':''}`} /></div></th>)}
          {visibleColumns.updatedAt && (<th className="sortable" onClick={()=>handleSort('updated_at')}><div className="th-content">{t('inventory.item_class_coding.list.updated')}<ArrowUpDown size={14} className={`sort-icon ${sortConfig.key==='updated_at'?'active':''}`} /></div></th>)}
