...
const COLUMN_LABEL_KEYS = {
  id: 'col_id',
  uomCode: 'col_uom_code',
  uomName: 'col_uom_name',
  uomType: 'col_uom_type',
  symbol: 'col_symbol',
  decimalPrecision: 'col_precision',
  status: 'col_status',
  updatedAt: 'col_updated',
};

const UOM_TYPES = ['Length', 'Weight', 'Volume', 'Quantity', 'Time', 'Area', 'Other'];
...
export default function List() {
  ...
  const typeLabel = useCallback(
    (typeKey) => {
      const tr = t(`inventory.uom_master.uom_types.${typeKey}`);
      return tr === `inventory.uom_master.uom_types.${typeKey}` ? typeKey : tr;
    },
    [t]
  );
  ...
  const visibleColumnsInit = useMemo(
    () => ({ id: true, uomCode: true, uomName: true, uomType: true, symbol: true, decimalPrecision: true, status: true, updatedAt: true, actions: true }),
    []
  );
  ...
  const handleBulkDelete = () => {
    ...
    CustomAlert.fire({
      ...
      onConfirm: () => {
        setLoading(true);
        router.post('/inventory/uom-master/bulk-destroy', { ids: selected }, {
  ...
  const handleDelete = (item) => {
    CustomAlert.fire({
      ...
      text: td('confirm_delete_text', { name: item.uom_name }),
      icon: 'warning',
      showCancelButton: true,
      ...
      onConfirm: () => {
        setLoading(true);
        router.delete(`/inventory/uom-master/${item.id}`, { onFinish: () => setLoading(false) });
      },
    });
  };
  ...
  const typeOptions = useMemo(
    () => [{ value: 'all', label: t('inventory.shared.filter_all_types') }, ...UOM_TYPES.map((ty) => ({ value: ty, label: typeLabel(ty) }))],
    [t, typeLabel]
  );
        ...
        <div className="manager-header">
          <div className="header-main">
            ...
            <div className="header-actions">
              ...
              <a href="/inventory/uom-master/create" className="btn btn-primary">
                <Plus size={20} />
                {tl('add_uom')}
              </a>
            </div>
              ...
              <div className="search-section">
                <div className="search-input-wrapper">
                  ...
                  <input
                    ...
                    placeholder={tl('search_uoms_by_code_name_or_symbol')}
                    value={searchTerm}
                    onChange={(e) => handleSearch(e.target.value)}
                  />
                </div>
                <div className="filter-group">
                  <label className="filter-label">{tl('type')}</label>
                  <select className="filter-select" value={typeFilter} onChange={(e) => handleTypeFilter(e.target.value)}>
                    {typeOptions.map((o) => (
        ...
        <div className="data-table-container">
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  {visibleColumns.uomCode && (
                    <th className="sortable" onClick={() => handleSort('uom_code')}>
                      <div className="th-content">
                        {tl('uom_code')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'uom_code' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                  {visibleColumns.uomName && (
                    <th className="sortable" onClick={() => handleSort('uom_name')}>
                      <div className="th-content">
                        {tl('uom_name')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'uom_name' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                  {visibleColumns.uomType && (
                    <th className="sortable" onClick={() => handleSort('uom_type')}>
                      <div className="th-content">
                        {tl('type')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'uom_type' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                  {visibleColumns.symbol && (
                    <th className="sortable" onClick={() => handleSort('symbol')}>
                      <div className="th-content">
                        {tl('symbol')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'symbol' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                  {visibleColumns.decimalPrecision && (
                    <th className="sortable" onClick={() => handleSort('decimal_precision')}>
                      <div className="th-content">
                        {tl('precision')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'decimal_precision' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                    ...
                    {visibleColumns.uomCode && (
                      <td>
                        <div className="module-details">
                          <div className="module-name">{item.uom_code}</div>
                        </div>
                      </td>
                    )}
                    {visibleColumns.uomName && (
                      <td>
                        <div className="module-details">
                          <div className="module-name">{item.uom_name}</div>
                        </div>
                      </td>
                    )}
                    {visibleColumns.uomType && (
                      <td>
                        <span className="badge badge-info">{typeLabel(item.uom_type)}</span>
                      </td>
                    )}
                    {visibleColumns.symbol && (
                      <td>
                        <span className="badge badge-secondary">{item.symbol}</span>
                      </td>
                    )}
                    {visibleColumns.decimalPrecision && (
                      <td>
                        <span className="badge badge-default">{item.decimal_precision}</span>
                      </td>
                    )}
                    {visibleColumns.status && (
                      <td>
                    ...
                    {visibleColumns.actions && (
                      <td>
                        <div className="actions-cell">
                          <button className="action-btn edit" title={tl('edit_uom')} onClick={() => router.get(`/inventory/uom-master/${item.id}/edit`)}>
                            <Edit3 size={16} />
                          </button>
                          <button className="action-btn delete" title={tl('delete_uom')} onClick={() => handleDelete(item)}>
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
          ...
          <div className="pagination-container">
            ...
            <div className="pagination-buttons">
              ...
              <span className="pagination-current">{tl('page_title', { current: paginatedItems.current_page, last: paginatedItems.last_page })}</span>
              <button className="btn btn-sm" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === paginatedItems.last_page}>
                <ChevronRight size={16} />
              </button>
              <button className="btn btn-sm" onClick={() => handlePageChange(paginatedItems.last_page)} disabled={currentPage === paginatedItems.last_page}>
                {tl('last')}
                <ChevronRight size={16} />
              </button>
            </div>
