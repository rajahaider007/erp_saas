...
const COLUMN_LABEL_KEYS = {
  id: 'col_id',
  groupCode: 'col_group_code',
  groupName: 'col_group_name',
  status: 'col_status',
...
export default function List() {
  ...
  const [sortConfig, setSortConfig] = useState({
    key: filters?.sort_by || 'group_code',
  ...
  const handleBulkDelete = () => {
    ...
    CustomAlert.fire({
      ...
      onConfirm: () => {
        setLoading(true);
        router.post('/inventory/item-group-coding/bulk-destroy', { ids: selected }, {
  ...
  const handleDelete = (item) => {
    CustomAlert.fire({
      ...
      text: td('confirm_delete_text', { name: item.group_name }),
      icon: 'warning',
      showCancelButton: true,
      ...
      onConfirm: () => {
        setLoading(true);
        router.delete(`/inventory/item-group-coding/${item.id}`, { onFinish: () => setLoading(false) });
      },
    });
  };
        ...
        <div className="manager-header">
          <div className="header-main">
            ...
            <div className="header-actions">
              ...
              <a href="/inventory/item-group-coding/create" className="btn btn-primary">
                <Plus size={20} />
                {tl('add_item_group')}
              </a>
            </div>
              ...
              <div className="search-section">
                <div className="search-input-wrapper">
                  ...
                  <input
                    ...
                    placeholder={tl('search_item_groups')}
                    value={searchTerm}
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  {visibleColumns.groupCode && (
                    <th className="sortable" onClick={() => handleSort('group_code')}>
                      <div className="th-content">
                        {tl('group_code')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'group_code' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                  {visibleColumns.groupName && (
                    <th className="sortable" onClick={() => handleSort('group_name')}>
                      <div className="th-content">
                        {tl('group_name')}
                        <ArrowUpDown size={14} className={`sort-icon ${sortConfig.key === 'group_name' ? 'active' : ''}`} />
                      </div>
                    </th>
                  )}
                    ...
                    {visibleColumns.groupCode && (
                      <td>
                        <div className="module-details">
                          <div className="module-name">{item.group_code}</div>
                        </div>
                      </td>
                    )}
                    {visibleColumns.groupName && (
                      <td>
                        <div className="module-details">
                          <div className="module-name">{item.group_name}</div>
                        </div>
                      </td>
                    )}
                    {visibleColumns.status && (
                      ...
                      <td>
                        <div className="actions-cell">
                          <button
                            ...
                            title={tl('edit_item_group')}
                            onClick={() => router.get(`/inventory/item-group-coding/${item.id}/edit`)}
                          >
                            <Edit3 size={16} />
                          </button>
                          <button className="action-btn delete" title={tl('delete_item_group')} onClick={() => handleDelete(item)}>
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
