...
export default function PurchaseOrderLinesReport() {
  ...
  const tr = useCallback((k, r = {}) => t(`inventory.reports.purchase_order_lines.${k}`, r), [t]);
  ...
  const exportBase = useCallback(() => {
    const qs = buildPurchaseOrderLinesQueryString({
      ...filters,
  ...
  const fetchData = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        const res = await fetch('/inventory/reports/purchase-order-lines/data', {
          method: 'POST',
          headers: {
  ...
  const breadcrumbItems = useMemo(
    () => [
      ...
      { label: tr('title'), href: '/inventory/reports/purchase-order-lines', icon: ClipboardList },
    ],
    [t, ts, tr]
  );
  ...
  const goChangeFilters = useCallback(() => {
    ...
    router.visit(`/inventory/reports/purchase-order-lines?${qs}`);
  }, [filters]);

  const handlePrint = useCallback(() => {
    window.open(`/inventory/reports/purchase-order-lines/print${exportBase()}`, '_blank', 'noopener,noreferrer');
  }, [exportBase]);
        ...
        <div className="manager-header mx-4 mt-2">
          <div className="header-main">
            <div className="title-section">
              ...
              <div className="stats-summary flex-wrap">
                {scopeChips.map((c) => (
                  <span
                    key={c}
                    className="stat-item rounded-full bg-slate-700/80 px-3 py-1 text-xs border border-slate-600"
                  >
                    {c}
                  </span>
                ))}
            ...
            <div className="header-actions flex-wrap">
              ...
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/purchase-order-lines/export/csv${exportBase()}`}>
                {tr('export_csv')}
              </a>
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/purchase-order-lines/export/excel${exportBase()}`}>
                {tr('export_excel')}
              </a>
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/purchase-order-lines/export/pdf${exportBase()}`}>
                {tr('export_pdf')}
              </a>
              ...
              <table className="data-table min-w-full text-sm">
                <thead>
                  <tr>
                    ...
                    <th className="text-right whitespace-nowrap">{tr('col_received')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_balance')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_unit_price')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_discount_pct')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_net_unit')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_open_value')}</th>
                ...
                <tbody>
                  ...
                  {!loading &&
                    rows.map((r) => (
                      <tr key={r.line_id}>
                        <td className="whitespace-nowrap font-mono text-xs">{r.po_number}</td>
                        ...
                        <td className="text-right font-medium">{fmtNum(r.balance_qty)}</td>
                        <td className="text-right">{fmtNum(r.unit_price, 6)}</td>
                        <td className="text-right">{fmtNum(r.line_discount_percent, 2)}</td>
                        <td className="text-right">{fmtNum(r.net_unit_price, 6)}</td>
                        <td className="text-right">{fmtNum(r.open_line_value, 2)}</td>
                        <td>{r.currency_code || '—'}</td>
                        <td className="text-xs">{r.line_status || '—'}</td>
                        <td className="text-xs">{r.po_status}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
