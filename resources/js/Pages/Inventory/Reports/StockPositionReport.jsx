...
import { ArrowLeft, Boxes, ClipboardList, Home, RefreshCw } from 'lucide-react';
...
export default function StockPositionReport() {
  ...
  const fetchData = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        const res = await fetch('/inventory/reports/stock-position/data', {
          method: 'POST',
          headers: {
  ...
  const goChangeFilters = useCallback(() => {
    const qs = buildStockPositionQueryString({ ...filters });
    router.visit(`/inventory/reports/stock-position?${qs}`);
  }, [filters]);
        ...
        <div className="manager-header mx-4 mt-2">
          <div className="header-main">
            ...
            <div className="header-actions flex-wrap">
              ...
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/stock-position/export/csv${exportBase()}`}>
                {tr('export_csv')}
              </a>
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/stock-position/export/excel${exportBase()}`}>
                {tr('export_excel')}
              </a>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-600/60 bg-slate-900/40 overflow-hidden shadow-lg">
            ...
            <div className="overflow-x-auto">
              <table className="data-table min-w-full text-sm">
                <thead>
                  <tr>
                    <th className="text-left whitespace-nowrap">{tr('col_location')}</th>
                    <th className="text-left whitespace-nowrap">{tr('col_item')}</th>
                    <th className="text-left whitespace-nowrap">{tr('col_uom')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_qty')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_value')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_avg_cost')}</th>
                  </tr>
                </thead>
                <tbody>
                  {loading && (
                  ...
                  {!loading &&
                    rows.map((r) => (
                      <tr key={`${r.at_location_id}-${r.inventory_item_id}-${r.uom_code}`}>
                        <td className="whitespace-nowrap max-w-[220px] truncate" title={r.location_name}>
                          {r.location_name || `#${r.at_location_id}`}
                        </td>
                        <td className="max-w-[260px] truncate" title={r.item_name}>
                          {r.item_code || '—'} {r.item_name ? `— ${r.item_name}` : ''}
                        </td>
                        <td>{r.uom_code || '—'}</td>
                        <td className="text-right font-medium">{fmtNum(r.qty_on_hand)}</td>
                        <td className="text-right">{fmtNum(r.value_on_hand, 2)}</td>
                        <td className="text-right">{r.avg_unit_cost != null ? fmtNum(r.avg_unit_cost, 6) : '—'}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
