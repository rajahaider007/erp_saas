...
import { ArrowLeft, ClipboardList, GitCompare, Home, RefreshCw } from 'lucide-react';
...
export default function GrnPoVarianceReport() {
  ...
  const [filters, setFilters] = useState(() => ({
    ...
    open_lines_only: Boolean(appliedFilters.open_lines_only),
    variance_only: Boolean(appliedFilters.variance_only),
    search: appliedFilters.search || '',
    sort_by: appliedFilters.sort_by || 'po_date',
    sort_order: appliedFilters.sort_order || 'desc',
  }));

  const [perPage, setPerPage] = useState(appliedFilters.per_page != null ? Number(appliedFilters.per_page) : 50);
  ...
  const exportBase = useCallback(() => {
    const qs = buildGrnPoVarianceQueryString({ ...filters });
    return `?${qs}`;
  }, [filters]);
    ...
    async (opts = {}) => {
      ...
      try {
        const body = {
          ...
          open_lines_only: filters.open_lines_only,
          variance_only: filters.variance_only,
        ...
        const res = await fetch('/inventory/reports/grn-po-variance/data', {
          method: 'POST',
          headers: {
  ...
  const goChangeFilters = useCallback(() => {
    const qs = buildGrnPoVarianceQueryString({ ...filters });
    router.visit(`/inventory/reports/grn-po-variance?${qs}`);
  }, [filters]);

  const scopeChips = useMemo(() => {
    const chips = [];
    if (filters.date_from || filters.date_to) chips.push(`${fmtDate(filters.date_from)} → ${fmtDate(filters.date_to)}`);
    if (filters.vendor_id) {
      const v = vendorOptions.find((o) => o.value === filters.vendor_id);
      chips.push(v ? v.label : filters.vendor_id);
    }
    if (filters.status !== 'all') chips.push(filters.status);
    if (filters.open_lines_only) chips.push(tr('open_lines_only'));
    if (filters.variance_only) chips.push(tr('variance_only'));
    ...
    <App>
      <div className="advanced-module-manager form-theme-system min-h-screen">
        ...
        <div className="manager-header mx-4 mt-2">
          <div className="header-main">
            ...
            <div className="header-actions flex-wrap">
              ...
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/grn-po-variance/export/csv${exportBase()}`}>
                {tr('export_csv')}
              </a>
              <a className="btn btn-secondary btn-sm" href={`/inventory/reports/grn-po-variance/export/excel${exportBase()}`}>
                {tr('export_excel')}
              </a>
            </div>
          </div>
        ...
        <div className="manager-content px-4 pb-10">
          <div className="rounded-xl border border-slate-600/60 bg-slate-900/40 overflow-hidden">
            <div className="flex flex-wrap items-end gap-3 px-4 py-3 border-b border-slate-600/60 bg-slate-800/50">
              ...
              <button type="button" className="btn btn-secondary btn-sm ml-auto" onClick={() => void fetchData({ page })} disabled={loading}>
              ...
              <table className="data-table min-w-full text-sm">
                <thead>
                  <tr>
                    ...
                    <th className="text-left whitespace-nowrap">{tr('col_po_date')}</th>
                    <th className="text-left whitespace-nowrap">{tr('col_expected')}</th>
                    ...
                    <th className="text-right whitespace-nowrap">{tr('col_ordered')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_po_received')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_grn_accepted')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_delta')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_po_net')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_grn_avg')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_price_var')}</th>
                    <th className="text-right whitespace-nowrap">{tr('col_recv_vs_ord')}</th>
                    <th className="text-left whitespace-nowrap">{tr('col_currency')}</th>
                    <th className="text-left whitespace-nowrap">{tr('col_line_status')}</th>
                    <th className="text-left whitespace-nowrap">{tr('col_po_status')}</th>
                  </tr>
                </thead>
                <tbody>
                  {loading && (
                  ...
                  {!loading &&
                    rows.map((r) => (
                      <tr key={r.line_id}>
                        <td className="font-mono text-xs whitespace-nowrap">{r.po_number}</td>
                        <td className="whitespace-nowrap">{fmtDate(r.po_date)}</td>
                        <td className="whitespace-nowrap">{fmtDate(r.expected_delivery_date)}</td>
                        <td className="max-w-[180px] truncate" title={`${r.vendor_code} — ${r.vendor_name}`}>
                        ...
                        <td>{r.uom_code || '—'}</td>
                        <td className="text-right">{fmtNum(r.ordered_qty)}</td>
                        <td className="text-right">{fmtNum(r.received_qty)}</td>
                        <td className="text-right">{fmtNum(r.grn_accepted_qty)}</td>
                        <td className="text-right">{fmtNum(r.grn_vs_po_received_delta)}</td>
                        <td className="text-right">{fmtNum(r.net_unit_price, 6)}</td>
                        <td className="text-right">{r.grn_avg_unit_cost != null ? fmtNum(r.grn_avg_unit_cost, 6) : '—'}</td>
                        <td className="text-right">{r.price_variance_pct != null ? fmtNum(r.price_variance_pct, 2) : '—'}</td>
                        <td className="text-right">{r.receipt_vs_ordered_pct != null ? fmtNum(r.receipt_vs_ordered_pct, 2) : '—'}</td>
            ...
            {meta.total > 0 && (
              <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-400 border-t border-slate-600/60">
                <span>
