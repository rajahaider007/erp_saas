...
export default function List() {
  ...
  const onDelete = (id) => {
    const confirmed = window.confirm(ml('confirm_delete'));
    if (!confirmed) return;
    ...
    <App>
      <div className="rounded-xl bg-white shadow p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {config?.key && (
            <a className="px-4 py-2 rounded-md bg-blue-600 text-white text-sm" href={`/inventory/master-data/${config.key}/create`}>
              {ml('add_new')}
            </a>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          ...
          />
          <select className="border rounded-md p-2" value={isActive} onChange={(e) => setIsActive(e.target.value)}>
            <option value="">{ml('all_status')}</option>
            <option value="1">{ml('active')}</option>
            <option value="0">{ml('inactive')}</option>
          </select>
          <button className="px-4 py-2 rounded-md border" onClick={() => pushQuery({ search, is_active: isActive })}>
            {ml('apply_filters')}
          </button>
        </div>
        ...
        <div className="overflow-auto border rounded-lg">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left p-3">{ml('id')}</th>
                {columns.map((column) => (
                  <th key={column.key} className="text-left p-3">
                    {column.label}
                  </th>
                ))}
                <th className="text-left p-3">{ml('status')}</th>
                <th className="text-left p-3">{ml('actions')}</th>
              </tr>
            </thead>
            <tbody>
              {(items?.data || []).map((row) => (
                <tr key={row.id} className="border-t">
                  ...
                  <td className="p-3">{row.is_active ? ts('active') : ts('inactive')}</td>
                  <td className="p-3">
                    <div className="flex gap-2">
                      <a className="px-2 py-1 rounded border" href={`/inventory/master-data/${config.key}/${row.id}/edit`}>
        ...
        <div className="mt-4 flex items-center gap-2">
          <button className="px-3 py-1 border rounded" disabled={!items?.prev_page_url} onClick={() => pushQuery({ page: String((items?.current_page || 1) - 1) })}>
            {ml('prev')}
          </button>
          <span className="text-sm text-gray-600">{ml('page_of', { current: items?.current_page || 1, last: items?.last_page || 1 })}</span>
          <button className="px-3 py-1 border rounded" disabled={!items?.next_page_url} onClick={() => pushQuery({ page: String((items?.current_page || 1) + 1) })}>
            {ml('next')}
          </button>
        </div>
      </div>
