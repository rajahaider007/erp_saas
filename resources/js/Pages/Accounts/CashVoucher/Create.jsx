...
const CashVoucherCreate = () => {
  const { accounts = [], CashAccounts = [], voucher = null, entries = [], flash, currencies = [], company = null, preview_voucher_number = null, preview_voucher_numbers = {}, attachments: initialAttachments = [], currentPeriod = null, attachmentPublicUrlMode = 'route' } = usePage().props;
  const { t } = useTranslations();
  ...
  const aiDraftPayload = useMemo(
    () => (!isEdit ? consumeRahjAiDraftPayload('cash_voucher') : null),
    [isEdit]
  );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.rate) {
          ...
          setAlert({ type: 'success', message: t('accounts.cash_voucher.create.msg_exchange_rate_updated_for_entry', { entry: entryIndex + 1, rate: data.rate }) });
        } else {
          setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_failed_to_fetch_exchange_rate_from_api') });
        }
      } else {
        setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_failed_to_fetch_exchange_rate_from_api') });
      }
    } catch (error) {
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_error_fetching_exchange_rate') + error.message });
    } finally {
  ...
  const handleAttachmentUpload = async (files) => {
    ...
    if (invalidFiles.length > 0) {
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_files_too_large', { files: invalidFiles.join(', ') }) });
      return;
    }
      ...
      if (response.ok) {
        ...
        setAttachments(prev => {
          ...
          return [...currentAttachments, ...newAttachments];
                        ...
                        setAlert({ type: 'success', message: t('accounts.cash_voucher.create.msg_attachments_uploaded_successfully', { count: validFiles.length }) });
        ...
        const errorData = await response.json().catch(() => ({ message: t('accounts.cash_voucher.create.msg_unknown_error') }));
        console.error('Upload failed:', errorData);
        setAlert({ type: 'error', message: errorData.message || t('accounts.cash_voucher.create.msg_failed_to_upload_attachments') });
      }
    } catch (error) {
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_error_uploading_attachments') + error.message });
    } finally {
  ...
  const handleEntryAttachmentUpload = async (entryIndex, files) => {
    ...
    if (validFiles.length > 1) {
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_only_one_attachment_per_entry_is_allowed') });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAlert({ type: 'success', message: t('accounts.cash_voucher.create.msg_attachment_uploaded_successfully_for_thi') });
        ...
        setAlert({ type: 'error', message: errorData.message || t('accounts.cash_voucher.create.msg_failed_to_upload_attachment') });
      }
    } catch (error) {
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_error_uploading_attachment') + error.message });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.voucher_date) {
      newErrors.voucher_date = t('accounts.cash_voucher.create.msg_voucher_date_required');
    }
    ...
    if (!autoVoucherNumbering && !isEdit) {
      if (!formData.voucher_number || !formData.voucher_number.trim()) {
        newErrors.voucher_number = t('accounts.cash_voucher.create.msg_voucher_number_required');
      }
    }
    ...
    if (!formData.voucher_sub_type) {
      newErrors.voucher_sub_type = t('accounts.cash_voucher.create.msg_voucher_type_required');
    }

    if (!formData.Cash_account_id) {
      newErrors.Cash_account_id = t('accounts.cash_voucher.create.msg_cash_account_required');
    }

    if (totalBaseAmount <= 0) {
      newErrors.entries = t('accounts.cash_voucher.create.msg_at_least_one_detail_amount_is_required');
    }
    ...
    formData.entries.forEach((entry, index) => {
      ...
      if (!entry.account_id) {
        newErrors[`entries.${index}.account_id`] = t('accounts.cash_voucher.create.msg_account_is_required');
        console.log(`Entry ${index}: Missing account_id`);
      }
      
      if (!entry.currency_code) {
        newErrors[`entries.${index}.currency_code`] = t('accounts.cash_voucher.create.msg_currency_is_required');
        console.log(`Entry ${index}: Missing currency_code`);
      }
      
      if (!entry.exchange_rate || entry.exchange_rate <= 0) {
        newErrors[`entries.${index}.exchange_rate`] = t('accounts.cash_voucher.create.msg_valid_exchange_rate_is_required');
      ...
      if (amount <= 0) {
        newErrors[`entries.${index}.amount`] = t('accounts.cash_voucher.create.msg_amount_is_required');
      }
      
      if (formData.Cash_account_id && parseInt(entry.account_id) === parseInt(formData.Cash_account_id)) {
        newErrors[`entries.${index}.account_id`] = t('accounts.cash_voucher.create.msg_detail_account_cannot_be_the_selected_cash_account');
      }
    });
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      setErrors(newErrors);
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_please_correct_the_errors_below') });
    ...
    try {
      ...
      if (isEdit) {
        router.put(`/accounts/cash-voucher/${voucher.id}`, submitData, {
          onSuccess: () => {
            setAlert({ type: 'success', message: t('accounts.cash_voucher.create.msg_cash_voucher_updated_successfully') });
        ...
        router.post('/accounts/cash-voucher', submitData, {
          onSuccess: () => {
            ...
            setAlert({ type: 'success', message: t('accounts.cash_voucher.create.msg_cash_voucher_created_successfully') });
      ...
      setAlert({ type: 'error', message: t('accounts.cash_voucher.create.msg_error_while_saving_voucher', { action: isEdit ? t('accounts.cash_voucher.create.action_updating') : t('accounts.cash_voucher.create.action_creating') }) });
      setIsSubmitting(false);
    }
  };

  const breadcrumbItems = [
    { label: t('accounts.cash_voucher.create.dashboard'), icon: Home, href: '/dashboard' },
    { label: t('accounts.cash_voucher.create.cash_vouchers'), icon: List, href: '/accounts/cash-voucher' },
    { label: isEdit ? t('accounts.cash_voucher.create.edit_cash_voucher') : t('accounts.cash_voucher.create.create_cash_voucher'), icon: FileText, href: null }
  ];
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-semibold mb-1">
                  {t('accounts.cash_voucher.create.accounting_period')}: {currentPeriod.period_name} (FY {currentPeriod.fiscal_year})
                </h3>
                <p className="text-sm mb-2">
                  <strong>{t('accounts.cash_voucher.create.status')}</strong> {currentPeriod.status}
                  {currentPeriod.is_adjustment_period && ` (${t('accounts.cash_voucher.create.adjustment_period')})`}
                </p>
                <p className="text-xs">
                  {currentPeriod.status === 'Open' && t('accounts.cash_voucher.create.period_open_description')}
                  {currentPeriod.status === 'Locked' && t('accounts.cash_voucher.create.period_locked_description')}
                  {currentPeriod.status === 'Closed' && t('accounts.cash_voucher.create.period_closed_description')}
                </p>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} onKeyDown={(e) => handleKeyDown(e)}>
              ...
              <div className="mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">
                  {isEdit ? t('accounts.cash_voucher.create.edit_cash_voucher') : t('accounts.cash_voucher.create.create_cash_voucher')}
                </h1>
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  {isEdit ? t('accounts.cash_voucher.create.subtitle_edit') : t('accounts.cash_voucher.create.subtitle_create')}
                </p>
              </div>
              ...
              <div className="mb-4">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2">
                    <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-3">{t('accounts.cash_voucher.create.cash_voucher_master')}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label htmlFor="voucher_number" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                          {t('accounts.cash_voucher.create.voucher_number_label')} {!autoVoucherNumbering && !isEdit && '*'}
                        </label>
                        <input
                          type="text"
                          id="voucher_number"
                          ...
                          placeholder={autoVoucherNumbering ? (formData.voucher_number || t('accounts.cash_voucher.create.auto_generated')) : t('accounts.cash_voucher.create.enter_voucher_number')}
                        ...
                        {autoVoucherNumbering && (
                          <p className="mt-0.5 text-xs text-blue-600 dark:text-blue-400">
                            {formData.voucher_number
                              ? t('accounts.cash_voucher.create.voucher_number_display', { number: formData.voucher_number })
                              : t('accounts.cash_voucher.create.auto_generated')}
                          </p>
                        )}
                      ...
                      <div>
                        <label htmlFor="voucher_sub_type" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                          {t('accounts.cash_voucher.create.voucher_type_required_label')}
                        </label>
                        <select
                          id="voucher_sub_type"
                        ...
                        >
                          <option value="Cash Payment">{t('accounts.cash_voucher.create.cash_payment')}</option>
                          <option value="Cash Receipt">{t('accounts.cash_voucher.create.cash_receipt')}</option>
                        </select>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                          {t('accounts.cash_voucher.create.cash_account_required_label')}
                        </label>
                        <InlineSearchSelect
                          ...
                          placeholder={t('accounts.cash_voucher.create.search_and_select_cash_account')}
                      ...
                      <div>
                        <label htmlFor="reference_number" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                          {t('accounts.cash_voucher.create.reference_number_label')}
                        </label>
                        <input
                          type="text"
                          id="reference_number"
                          ...
                          onKeyDown={handleKeyDown}
                          placeholder={t('accounts.cash_voucher.create.enter_reference_number')}
                          tabIndex={3}
                      ...
                      <div className="md:col-span-2">
                        <label htmlFor="description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                          {t('accounts.cash_voucher.create.description_label')}
                        </label>
                        <textarea
                          id="description"
                          name="description"
                          ...
                          onKeyDown={handleKeyDown}
                          placeholder={t('accounts.cash_voucher.create.enter_voucher_description')}
                          rows={3}
                          tabIndex={5}
                          maxLength={250}
                        ...
                        <div className="flex justify-between items-center mt-1">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            ...
                            {t('accounts.cash_voucher.create.characters_count', { count: 250 })}
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="lg:col-span-1">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                      <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-3 flex items-center gap-2">
                        ...
                        {t('accounts.cash_voucher.create.attachments_label')}
                      </h3>
                      ...
                      <div className="space-y-3">
                          ...
                          >
                            {uploadingAttachments ? (
                              <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400">
                                ...
                                <span className="text-sm">{t('accounts.cash_voucher.create.uploading')}</span>
                              </div>
                            ) : (
                              <div className="flex flex-col items-center gap-2">
                                ...
                                <div className="text-center">
                                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('accounts.cash_voucher.create.click_to_upload_files')}</p>
                                  <p className="text-xs text-gray-500 dark:text-gray-400">{t('accounts.cash_voucher.create.max_300kb_each')}</p>
                                </div>
                              </div>
                            )}
                          </label>
                        ...
                        <div className="mt-2">
                          <span className="text-xs text-gray-500 dark:text-gray-400">
                            <strong>{t('accounts.cash_voucher.create.supported_formats')}</strong> {t('accounts.cash_voucher.create.supported_file_types')}
                          </span>
                        </div>
                        ...
                        {Array.isArray(attachments) && attachments.length > 0 && (
                          <div className="space-y-2">
                            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('accounts.cash_voucher.create.uploaded_files')}</h4>
                              ...
                              <div key={attachment.id || attachment.original_name || attachment || idx} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                ...
                                <div className="flex items-center gap-1 flex-shrink-0">
                                  <a
                                    ...
                                    title={t('accounts.cash_voucher.create.view_file')}
                                  >
                                  ...
                                  <button
                                    ...
                                    title={t('accounts.cash_voucher.create.remove_file')}
                                  >
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">{t('accounts.cash_voucher.create.cash_detail_entries')}</h3>
                  <button
                    type="button"
                    onClick={addEntry}
                    title={t('accounts.cash_voucher.create.add_entry_alta')}
                  ...
                  >
                    <Plus size={16} />
                    {t('accounts.cash_voucher.create.add_entry')}
                  </button>
                </div>
                  ...
                  {formData.entries.map((entry, index) => (
                    <div key={index} className="border border-gray-300 dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700/50">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="text-xs font-semibold text-gray-700 dark:text-gray-300">{t('accounts.cash_voucher.create.entry_number', { number: index + 1 })}</h4>
                        {formData.entries.length > 1 && (
                          <button
                            type="button"
                            onClick={() => removeEntry(index)}
                            className="p-1 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors"
                            title={t('accounts.cash_voucher.create.remove_entry_delete')}
                            tabIndex={-1}
                          >
                            <Trash2 size={16} />
                          </button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        <div className="md:col-span-2">
                          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {t('accounts.cash_voucher.create.account_required_label')}
                          </label>
                          <InlineSearchSelect
                            options={accounts
                            ...
                            placeholder={t('accounts.cash_voucher.create.search_and_select_account')}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {t('accounts.cash_voucher.create.currency_label')}
                          </label>
                          <InlineSearchSelect
                            ...
                            placeholder={t('accounts.cash_voucher.create.search_and_select_currency')}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {t('accounts.cash_voucher.create.exchange_rate_label')}
                          </label>
                          <div className="flex gap-1">
                            <input
                              ...
                              placeholder={t('accounts.cash_voucher.create.1000000')}
                            />
                            <button
                              type="button"
                              ...
                              title={t('accounts.cash_voucher.create.fetch_current_exchange_rate')}
                            >
                            ...
                            <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                              {entry.currency_code !== formData.base_currency_code ? (
                                ...
                                t('accounts.cash_voucher.create.base_currency')
                              )}
                            </div>
                        </div>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {t('accounts.cash_voucher.create.amount_required_label')} ({entry.currency_code})
                          </label>
                          <input
                            type="number"
                            step="0.01"
                            ...
                            placeholder={t('accounts.cash_voucher.create.000')}
                          ...
                          {entry.amount && entry.currency_code !== formData.base_currency_code && (
                            <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                              {t('accounts.cash_voucher.create.base_value')}: {entry.base_amount} {formData.base_currency_code}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="md:col-span-4">
                          ...
                          <input
                            ...
                            onKeyDown={handleKeyDown}
                            placeholder={t('accounts.cash_voucher.create.entry_description_optional')}
                        ...
                        <div className="md:col-span-4">
                            ...
                            >
                              {entry.attachment ? (
                                ...
                                <div className="flex flex-col items-center gap-2">
                                  ...
                                  <div className="text-center">
                                    <p className="text-xs font-medium text-gray-900 dark:text-gray-100">{t('accounts.cash_voucher.create.click_to_upload_attachment')}</p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('accounts.cash_voucher.create.max_300kb__one_file_per_entry')}</p>
                                  </div>
                                </div>
                              )}
                            </label>
                          ...
                          <div className="mt-1">
                            <span className="text-xs text-gray-500 dark:text-gray-400">
                              <strong>{t('accounts.cash_voucher.create.supported')}</strong> PDF, DOC, DOCX, XLS, XLSX, JPG, JPEG, PNG, GIF
                            </span>
                          </div>
                        </div>
                      </div>
              ...
              <div className="mb-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/10 dark:to-purple-900/10 rounded-lg p-3 border border-blue-200 dark:border-blue-800">
                <div className="mb-3">
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">
                    {t('accounts.cash_voucher.create.base_currency_summary')} ({formData.base_currency_code})
                  </h4>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      <span className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-0.5">{t('accounts.cash_voucher.create.detail_total')}</span>
                      <span className="text-lg font-bold text-gray-900 dark:text-gray-100">
                        {totalBaseAmount.toFixed(2)} {formData.base_currency_code}
                      </span>
                    </div>
                    <div>
                      <span className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-0.5">{t('accounts.cash_voucher.create.voucher_type')}</span>
                      <span className="text-lg font-bold text-gray-900 dark:text-gray-100">
                        {formData.voucher_sub_type}
                      </span>
                    </div>
                    <div className="rounded-lg px-2 py-1 bg-green-100 dark:bg-green-900/30">
                      <span className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-0.5">{t('accounts.cash_voucher.create.auto_cash_contra')}</span>
                ...
                {(() => {
                  ...
                  return Object.keys(currencyBreakdown).length > 1 && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">
                        {t('accounts.cash_voucher.create.multi_currency_breakdown')}
                      </h4>
                      <div className="space-y-2">
                        {Object.entries(currencyBreakdown).map(([currency, amounts]) => (
                          <div key={currency} className="grid grid-cols-2 gap-3 text-center text-sm">
                            <div>
                              <span className="block text-xs font-medium text-gray-600 dark:text-gray-400">{t('accounts.cash_voucher.create.amount')}</span>
                              <span className="font-semibold text-gray-900 dark:text-gray-100">
                                {amounts.amount.toFixed(2)} {currency}
                              </span>
                            </div>
                            <div className="rounded px-2 py-1 bg-green-100 dark:bg-green-900/30">
                              <span className="block text-xs font-medium text-gray-600 dark:text-gray-400">{t('accounts.cash_voucher.create.auto_contra')}</span>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  <kbd className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded border border-gray-300 dark:border-gray-600">{t('accounts.cash_voucher.create.tab')}</kbd>{t('accounts.cash_voucher.create.or')}<kbd className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded border border-gray-300 dark:border-gray-600">{t('accounts.cash_voucher.create.enter')}</kbd> {t('accounts.cash_voucher.create.to_navigate')} • 
                  <kbd className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded border border-gray-300 dark:border-gray-600 ml-1">{t('accounts.cash_voucher.create.alta')}</kbd> {t('accounts.cash_voucher.create.to_add_entry')}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  >
                    <ArrowLeft size={16} />
                    {t('accounts.cash_voucher.create.cancel')}
                  </button>
                  <button
                    type="submit"
                  ...
                  >
                    {isSubmitting ? (
                      <>
                        ...
                        {isEdit ? t('accounts.cash_voucher.create.updating') : t('accounts.cash_voucher.create.creating')}
                      </>
                    ) : (
                      <>
                        <Save size={16} />
                        {isEdit ? t('accounts.cash_voucher.create.update') : t('accounts.cash_voucher.create.create')} <kbd className="ml-1 px-1.5 py-0.5 bg-blue-700 rounded text-xs">{t('accounts.cash_voucher.create.ctrls')}</kbd>
                      </>
                    )}
                  </button>
                </div>
