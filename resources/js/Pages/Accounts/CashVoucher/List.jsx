...
const CashVoucherList = () => {
  ...
  const handleView = (voucher) => {
    router.visit(`/accounts/cash-voucher/${voucher.id}`);
  };
  ...
  const handleBulkAction = (action) => {
    ...
    if (action === 'delete') {
      CustomAlert.fire({
        ...
        text: `You are about to delete ${selectedVouchers.length} Cash voucher(s). This action cannot be undone!`,
        icon: 'warning',
        showCancelButton: true,
        ...
        onConfirm: () => {
          ...
          selectedVouchers.forEach(id => {
            router.delete(`/accounts/cash-voucher/${id}`);
          });
          setSelectedVouchers([]);
          setLoading(false);
        }
    ...
    if (action === 'post') {
      ...
      CustomAlert.fire({
        ...
        text: `You are about to post ${selectedVouchers.length} Cash voucher(s). This action cannot be undone!`,
        icon: 'question',
        showCancelButton: true,
        ...
        onConfirm: () => {
          setLoading(true);
          router.post('/accounts/cash-voucher/bulk-post', {
  ...
  const exportToCSV = () => {
    CustomAlert.fire({
      title: 'Export to CSV',
      text: 'Download all Cash vouchers as CSV file?',
      icon: 'question',
      showCancelButton: true,
      ...
      onConfirm: () => {
        ...
        const params = new URLSearchParams(window.location.search);
        const exportUrl = `/accounts/cash-voucher/export-csv?${params.toString()}`;
  ...
  const exportToExcel = () => {
    CustomAlert.fire({
      ...
      text: 'Download all Cash vouchers as Excel file?',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Yes, download!',
      cancelButtonText: 'Cancel',
      onConfirm: () => {
        // Build query parameters for current filters
        const params = new URLSearchParams(window.location.search);
        const exportUrl = `/accounts/cash-voucher/export-excel?${params.toString()}`;
        window.open(exportUrl, '_blank');
      }
    });
  };
  ...
  const exportToPDF = () => {
    CustomAlert.fire({
      title: 'Export to PDF',
      text: 'Download all Cash vouchers as PDF file?',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Yes, download!',
      cancelButtonText: 'Cancel',
      onConfirm: () => {
        // Build query parameters for current filters
        const params = new URLSearchParams(window.location.search);
        const exportUrl = `/accounts/cash-voucher/export-pdf?${params.toString()}`;
        window.open(exportUrl, '_blank');
      }
    });
  };
          ...
          <div className="header-main">
            <div className="title-section">
              <h1 className="page-title">
                ...
                {usePage().props?.pageTitle || 'Cash Vouchers'}
              </h1>
            ...
            <div className="header-actions">
              <button
                ...
                title={t('accounts.cash_voucher.list.refresh')}
                disabled={loading}
              >
              ...
              <a href='/accounts/cash-voucher/create' className="btn btn-primary">
                <Plus size={20} />
                Add Cash Voucher
              </a>
            </div>
            ...
            <div className="search-section">
              <div className="search-input-wrapper">
                ...
                <input
                  ...
                  placeholder={t('accounts.cash_voucher.list.search_vouchers')}
                  value={searchTerm}
              ...
              <div className="filter-group">
                <label className="filter-label">{t('accounts.cash_voucher.list.status')}</label>
                <select
              ...
              <div className="filter-group">
                <label className="filter-label">{t('accounts.cash_voucher.list.quick')}</label>
                <select
                  className="filter-select"
                  value={getCurrentQuickFilter()}
                  onChange={(e) => handleQuickFilterChange(e.target.value)}
                >
                  <option value="">{t('accounts.cash_voucher.list.all_time')}</option>
                  <option value="today">{t('accounts.cash_voucher.list.today')}</option>
                  <option value="thisWeek">{t('accounts.cash_voucher.list.this_week')}</option>
                  <option value="thisMonth">{t('accounts.cash_voucher.list.this_month')}</option>
                </select>
              </div>

              <div className="filter-group">
                <label className="filter-label">{t('accounts.cash_voucher.list.columns')}</label>
                <select
                  className="filter-select"
                  value=""
                  onChange={(e) => handleColumnToggle(e.target.value)}
                >
                  <option value="">{t('accounts.cash_voucher.list.showhide')}</option>
              ...
              <button
                ...
                title={t('accounts.cash_voucher.list.reset_all_filters')}
              >
                <RefreshCcw size={16} />
              </button>
            </div>
          ...
          <div className="data-table-container">
            {loading ? (
              <div className="loading-state">
                ...
                <p>{t('accounts.cash_voucher.list.loading_cash_vouchers')}</p>
              </div>
            ) : !paginatedVouchers?.data?.length ? (
              <div className="empty-state">
                <Database className="empty-icon" />
                <h3>{t('accounts.cash_voucher.list.no_cash_vouchers_found')}</h3>
                <p>{t('accounts.cash_voucher.list.try_adjusting_your_filters_or_search_cri')}</p>
                <a href="/accounts/cash-voucher/create" className="btn btn-primary">
                  <Plus size={20} />
                  Add Your First Cash Voucher
                </a>
              </div>
            ) : (
              <>
                    ...
                    <thead>
                      <tr>
                        ...
                        {visibleColumns.actions && (
                          <th className="actions-header">{t('accounts.cash_voucher.list.actions')}</th>
                        )}
                      </tr>
                    </thead>
                    <tbody>
                            ...
                            <td>
                              <div className="actions-cell">
                                ...
                                <button
                                  ...
                                  title={t('accounts.cash_voucher.list.print_summary')}
                                ...
                                <button
                                  ...
                                  title={t('accounts.cash_voucher.list.print_detailed')}
                                ...
                                {voucher.status === 'Draft' && (
                                  <>
                                    <button
                                      ...
                                      title={t('accounts.cash_voucher.list.edit_voucher')}
                                    ...
                                    <button
                                      ...
                                      title={t('accounts.cash_voucher.list.post_voucher')}
                                    ...
                                    <button
                                      ...
                                      title={t('accounts.cash_voucher.list.delete_voucher')}
                ...
                <div className="pagination-container">
                  <div className="pagination-info">
                    ...
                    <div className="page-size-selector">
                      <span>{t('accounts.cash_voucher.list.show')}</span>
                      <select
                        value={pageSize}
                      ...
                      <span>{t('accounts.cash_voucher.list.per_page')}</span>
                    </div>
                  </div>
                  ...
                  <div className="pagination-controls">
                    <button
                      ...
                      title={t('accounts.cash_voucher.list.first_page')}
                    >
                      <ChevronLeft size={14} />
                      <ChevronLeft size={14} />
                    </button>
                    ...
                    <button
                      ...
                      title={t('accounts.cash_voucher.list.previous_page')}
                    >
                      <ChevronLeft size={14} />
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => handlePageChange(currentPage + 1)}
                      title={t('accounts.cash_voucher.list.next_page')}
                    >
                    ...
                    <button
                      ...
                      title={t('accounts.cash_voucher.list.last_page')}
                    >
                      <ChevronRight size={14} />
                      <ChevronRight size={14} />
                    </button>
                  ...
                  <div className="quick-jump">
                    <span>{t('accounts.cash_voucher.list.go_to')}</span>
                    <input
                      type="number"
                      min="1"
