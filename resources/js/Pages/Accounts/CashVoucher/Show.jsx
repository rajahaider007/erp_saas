...
const { t } = useTranslations();
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            ...
            >
              ...
              {t('accounts.cash_voucher.show.back_to_vouchers')}
            </button>
          </div>
        </div>
      </App>
  ...
  const handleDelete = () => {
    ...
    if (confirm(t('accounts.cash_voucher.show.confirm_delete'))) {
  ...
  const handlePost = () => {
    ...
    if (confirm(t('accounts.cash_voucher.show.confirm_post'))) {
            ...
            <div className="flex items-center space-x-3">
              <div className="relative group">
                <button className="inline-flex items-center px-3 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                  ...
                  {t('accounts.cash_voucher.show.print')}
                </button>
                <div className="absolute left-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                  ...
                  >
                    ...
                    {t('accounts.cash_voucher.show.print_summary')}
                  </button>
                  <button
                  ...
                  >
                    <FileText className="h-4 w-4 mr-2" />
                    {t('accounts.cash_voucher.show.print_detailed')}
                  </button>
                </div>
              </div>
              <button className="inline-flex items-center px-3 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                <Download className="h-4 w-4 mr-2" />
                {t('accounts.cash_voucher.show.export_pdf')}
              </button>
              {voucher.status === 'Draft' && (
                <>
                  ...
                  >
                    ...
                    {t('accounts.cash_voucher.show.edit')}
                  </button>
                  <button
                    onClick={handlePost}
                  ...
                  >
                    ...
                    {t('accounts.cash_voucher.show.post')}
                  </button>
                  <button
                  ...
                  >
                    ...
                    {t('accounts.cash_voucher.show.delete')}
                  </button>
                </>
              )}
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-semibold mb-1">
                  {t('accounts.cash_voucher.show.accounting_period')}: {currentPeriod.period_name} (FY {currentPeriod.fiscal_year})
                </h3>
                <p className="text-sm mb-2">
                  <strong>{t('accounts.cash_voucher.show.status')}</strong> {currentPeriod.status}
                  {currentPeriod.is_adjustment_period && ` (${t('accounts.cash_voucher.show.adjustment_period')})`}
                </p>
                <p className="text-xs">
                  {currentPeriod.status === 'Open' && t('accounts.cash_voucher.show.period_open_description')}
                  {currentPeriod.status === 'Locked' && t('accounts.cash_voucher.show.period_locked_description')}
                  {currentPeriod.status === 'Closed' && t('accounts.cash_voucher.show.period_closed_description')}
                </p>
              </div>
            </div>
          </div>
              ...
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {t('accounts.cash_voucher.show.line')}
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {t('accounts.cash_voucher.show.account')}
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {t('accounts.cash_voucher.show.description')}
                  </th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {t('accounts.cash_voucher.show.debit')}
                  </th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {t('accounts.cash_voucher.show.credit')}
                  </th>
                </tr>
              </thead>
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  <td colSpan="3" className="px-6 py-4 text-sm font-medium text-gray-900">
                    {t('accounts.cash_voucher.show.total')}
                  </td>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              ...
              <p className="text-sm text-gray-900">{t('accounts.cash_voucher.show.user_id_prefix')}: {voucher.created_by}</p>
            </div>
            <div>
            ...
            {voucher.posted_by && (
              <div>
                ...
                <p className="text-sm text-gray-900">{t('accounts.cash_voucher.show.user_id_prefix')}: {voucher.posted_by}</p>
              </div>
            )}
            {voucher.posted_at && (
              <div>
