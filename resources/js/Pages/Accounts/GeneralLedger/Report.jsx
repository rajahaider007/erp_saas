...
const GeneralLedgerReport = () => {
  ...
  const handlePrint = () => {
    ...
    window.open(`/accounts/general-ledger/print?${params.toString()}`, '_blank');
  };

  const handleExportPDF = () => {
    const params = new URLSearchParams(window.location.search);
    window.open(`/accounts/general-ledger/export-pdf?${params.toString()}`, '_blank');
  };

  const handleExportExcel = () => {
    const params = new URLSearchParams(window.location.search);
    window.open(`/accounts/general-ledger/export-excel?${params.toString()}`, '_blank');
  };

  const handleExportAllDataTables = () => {
    // Export all DataTables data to Excel
    if (window.$ && window.$.fn.DataTable) {
      let allData = [];
