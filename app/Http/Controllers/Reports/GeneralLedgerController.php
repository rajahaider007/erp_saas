...
class GeneralLedgerController extends Controller
{
    ...
    private function generateCSV($reportData, $filters, $visibleColumns)
    {
        ...
        // Add headers
        $headers = [];
        if ($visibleColumns['account_code']) {
            $headers[] = 'Account Code';
        }
        if ($visibleColumns['account_name']) {
            $headers[] = 'Account Name';
        }
        if ($visibleColumns['account_type']) {
            $headers[] = 'Type';
        }
        if ($visibleColumns['opening_balance']) {
            $headers[] = 'Opening Balance';
        }
        if ($visibleColumns['debit_total']) {
            $headers[] = 'Debit Total';
        }
        if ($visibleColumns['credit_total']) {
            $headers[] = 'Credit Total';
        }
        if ($visibleColumns['closing_balance']) {
            $headers[] = 'Closing Balance';
        }
        if ($visibleColumns['transactions_count']) {
            $headers[] = 'Transactions';
        }
        if ($visibleColumns['last_transaction_date']) {
            $headers[] = 'Last Transaction';
        }
        ...
        foreach ($reportData as $row) {
            $csvRow = [];
            if ($visibleColumns['account_code']) {
                $csvRow[] = $row['account_code'];
            }
            if ($visibleColumns['account_name']) {
                $csvRow[] = $row['account_name'];
            }
            if ($visibleColumns['account_type']) {
                $csvRow[] = $row['account_type'];
            }
            if ($visibleColumns['opening_balance']) {
                $csvRow[] = $row['opening_balance'];
            }
            if ($visibleColumns['debit_total']) {
                $csvRow[] = $row['debit_total'];
            }
            if ($visibleColumns['credit_total']) {
                $csvRow[] = $row['credit_total'];
            }
            if ($visibleColumns['closing_balance']) {
                $csvRow[] = $row['closing_balance'];
            }
            if ($visibleColumns['transactions_count']) {
                $csvRow[] = $row['transactions_count'];
            }
            if ($visibleColumns['last_transaction_date']) {
                $csvRow[] = $row['last_transaction_date'];
            }
    ...
    private function generateReportHTML($reportData, $filters, $visibleColumns)
    {
        ...
        if ($visibleColumns['opening_balance']) {
            $html .= '<th class="text-right">Opening Balance</th>';
        }
        if ($visibleColumns['debit_total']) {
            $html .= '<th class="text-right">Debit Total</th>';
        }
        if ($visibleColumns['credit_total']) {
            $html .= '<th class="text-right">Credit Total</th>';
        }
        if ($visibleColumns['closing_balance']) {
            $html .= '<th class="text-right">Closing Balance</th>';
        }
        if ($visibleColumns['transactions_count']) {
            $html .= '<th class="text-center">Transactions</th>';
        }
        if ($visibleColumns['last_transaction_date']) {
        ...
        foreach ($reportData as $row) {
            ...
            if ($visibleColumns['opening_balance']) {
                $html .= '<td class="text-right currency">'.number_format($row['opening_balance'], 2).'</td>';
            }
            if ($visibleColumns['debit_total']) {
                $html .= '<td class="text-right currency">'.number_format($row['debit_total'], 2).'</td>';
            }
            if ($visibleColumns['credit_total']) {
                $html .= '<td class="text-right currency">'.number_format($row['credit_total'], 2).'</td>';
            }
            if ($visibleColumns['closing_balance']) {
                $html .= '<td class="text-right currency">'.number_format($row['closing_balance'], 2).'</td>';
            }
            if ($visibleColumns['transactions_count']) {
                $html .= '<td class="text-center">'.$row['transactions_count'].'</td>';
            }
            if ($visibleColumns['last_transaction_date']) {
                $html .= '<td>'.($row['last_transaction_date'] ? date('Y-m-d', strtotime($row['last_transaction_date'])) : '-').'</td>';
            }
            $html .= '</tr>';
        }
