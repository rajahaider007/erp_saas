...
class OpeningVoucherController extends Controller
{
    use CheckUserPermissions;
    /**
     * Display a listing of Opening vouchers
     */
    public function index(Request $request)
    {
        ...
        if (!$compId || !$locationId) {
            return Inertia::render('Accounts/OpeningVoucher/List', [
                'OpeningVouchers' => [
                    'data' => [],
                    'total' => 0,
                    'current_page' => 1,
        ...
        // Paginate results
        $OpeningVouchers = $query->paginate($perPage);
    ...
    /**
     * Show the form for creating a new Opening voucher
     */
    public function create(Request $request)
    {
        ...
        if (!$compId || !$locationId) {
            return Inertia::render('Accounts/OpeningVoucher/Create', [
                'accounts' => [],
                'currencies' => [],
                'company' => null,
                'error' => 'Company and Location information is required. Please contact administrator.'
        ...
        $previewVoucherNumber = $this->generatePreviewVoucherNumber($compId, $locationId, 'Opening');
    ...
    /**
     * Store a newly created Opening voucher
     * 
    ...
    public function store(Request $request)
    {
        // Check if user has permission to can_add
        $this->requirePermission($request, null, 'can_add');
        Log::info('=== Opening VOUCHER STORE METHOD CALLED ===');
        ...
        try {
            ...
            Log::warning('Opening voucher validation failed', [
                'errors' => $e->errors(),
        ...
        // Always auto-generate voucher number for Opening Vouchers
        $company = DB::table('companies')->where('id', $compId)->first();
        ...
        $voucherNumber = $this->generateVoucherNumber($compId, $locationId, 'Opening');
        ...
        try {
            ...
            Log::info('Opening voucher created successfully', [
                'transaction_id' => $transactionId,
                'voucher_number' => $voucherNumber,
                'total_debit' => $totalBaseDebit,
                'total_credit' => $totalBaseCredit,
            ...
            // Create audit log for the Opening voucher creation
            try {
                $voucherData = [
                    'voucher_number' => $voucherNumber,
                    'voucher_date' => $request->voucher_date,
                ...
                AuditLogService::logOpeningVoucher('CREATE', $transactionId, $voucherData);
                Log::info('Audit log created for Opening voucher creation', ['transaction_id' => $transactionId]);
            } catch (\Exception $auditException) {
                // Don't fail the main operation if audit logging fails
                Log::warning('Failed to create audit log for Opening voucher creation', [
                    'transaction_id' => $transactionId,
            ...
            return redirect()->route('accounts.Opening-voucher.index')
                ->with('success', 'Opening voucher created successfully!');
        } catch (\Illuminate\Database\QueryException $e) {
            DB::rollback();

            // Log detailed error for developers
            Log::error('Database error creating Opening voucher', [
                'error' => $e->getMessage(),
                'sql' => $e->getSql(),
            ...
            return redirect()->back()->with('error', 'There was a problem saving your Opening voucher. Please check your data and try again.');
        } catch (\Exception $e) {
            DB::rollback();

            // Log detailed error for developers
            Log::error('Error creating Opening voucher', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'request_data' => $request->all(),
                'user_id' => $userId,
            ...
            // Return user-friendly error message
            return redirect()->back()->with('error', 'Something went wrong while creating the Opening voucher. Please try again or contact support if the problem persists.');
        }
    }

    /**
     * Display the specified Opening voucher
     */
    public function show(Request $request, $id)
    {
        ...
        if (!$compId || !$locationId) {
            return Inertia::render('Accounts/OpeningVoucher/Show', [
                'voucher' => null,
                'entries' => [],
                'error' => 'Company and Location information is required. Please contact administrator.'
            ]);
        ...
        if (!$voucher) {
            abort(404, 'Opening voucher not found');
        }
    ...
    /**
     * Show the form for editing the specified Opening voucher
     */
    ...
    /**
     * Update the specified Opening voucher
     */
    public function update(Request $request, $id)
    {
        ...
        if (!$voucher) {
            return redirect()->back()->with('error', 'Opening voucher not found');
        }
        ...
        try {
            ...
            // Create audit log for the Opening voucher update
            try {
                $voucherData = [
                ...
                AuditLogService::logOpeningVoucher('UPDATE', $id, $voucherData, $oldData);
                Log::info('Audit log created for Opening voucher update', ['transaction_id' => $id]);
            } catch (\Exception $auditException) {
                // Don't fail the main operation if audit logging fails
                Log::warning('Failed to create audit log for Opening voucher update', [
                    'transaction_id' => $id,
                    'error' => $auditException->getMessage()
                ]);
            }
            ...
            return redirect()->route('accounts.Opening-voucher.index')
                ->with('success', 'Opening voucher updated successfully!');
        } catch (\Illuminate\Database\QueryException $e) {
            DB::rollback();

            // Log detailed error for developers
            Log::error('Database error updating Opening voucher', [
                'error' => $e->getMessage(),
                'sql' => $e->getSql(),
                'bindings' => $e->getBindings(),
                'trace' => $e->getTraceAsString(),
            ...
            // Return user-friendly error message
            return redirect()->back()->with('error', 'There was a problem updating your Opening voucher. Please check your data and try again.');
        } catch (\Exception $e) {
            DB::rollback();

            // Log detailed error for developers
            Log::error('Error updating Opening voucher', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'request_data' => $request->all(),
                'voucher_id' => $id,
            ...
            // Return user-friendly error message
            return redirect()->back()->with('error', 'Something went wrong while updating the Opening voucher. Please try again or contact support if the problem persists.');
        }
    }

    /**
     * Get transactional accounts (level 4) for Opening entries
     */
    ...
    /**
     * Export Opening vouchers to CSV
     */
    public function exportCsv(Request $request)
    {
        ...
        $filename = 'Opening_vouchers_' . date('Y-m-d_His') . '.csv';
    ...
    /**
     * Export Opening vouchers to Excel
     */
    public function exportExcel(Request $request)
    {
        ...
        $filename = 'Opening_vouchers_' . date('Y-m-d_His') . '.xlsx';
    ...
    /**
     * Export Opening vouchers to PDF
     */
    public function exportPdf(Request $request)
    {
        ...
        $html .= '<h1>Opening Vouchers Report</h1>';
        ...
        return response($html)
            ...
            ->header('Content-Disposition', 'attachment; filename="Opening_vouchers_' . date('Y-m-d_His') . '.pdf"');
    }

    /**
     * Post a single Opening voucher (change status from Draft to Posted)
     */
    public function post(Request $request, $id)
    {
        ...
        try {
            ...
            // Create audit log for the Opening voucher posting
            try {
                $voucherData = [
                    'voucher_number' => $voucher->voucher_number,
                    'voucher_date' => $voucher->voucher_date,
                ...
                AuditLogService::logOpeningVoucher('POST', $id, $voucherData, $oldData);
                Log::info('Audit log created for Opening voucher posting', ['transaction_id' => $id]);
            } catch (\Exception $auditException) {
                // Don't fail the main operation if audit logging fails
                Log::warning('Failed to create audit log for Opening voucher posting', [
                    'transaction_id' => $id,
                    'error' => $auditException->getMessage()
                ]);
            }
            ...
            return redirect()->route('accounts.Opening-voucher.index')
                ->with('success', "Opening voucher {$voucher->voucher_number} posted successfully!");
        } catch (\Exception $e) {
            Log::error('Error posting Opening voucher', [
                'error' => $e->getMessage(),
                'voucher_id' => $id,
                'comp_id' => $compId,
                'location_id' => $locationId
    ...
    public function bulkPost(Request $request)
    {
        ...
        try {
            ...
            Log::error('Error in bulk post Opening vouchers', [
                'error' => $e->getMessage(),
    ...
    public function printSummary(Request $request, $id)
    {
        ...
        return Inertia::render('Accounts/OpeningVoucher/PrintSummary', [
            'voucher' => $voucher,
            'entries' => $entries,
            'company' => $company
        ]);
    ...
    /**
     * Print Opening voucher detailed
     */
    public function printDetailed(Request $request, $id)
    {
        ...
        return Inertia::render('Accounts/OpeningVoucher/PrintDetailed', [
            'voucher' => $voucher,
            'entries' => $entries,
            'company' => $company
        ]);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        Log::info('=== Opening VOUCHER DESTROY METHOD CALLED ===', ['voucher_id' => $id]);
        ...
        if (!$compId || !$locationId || !$userId) {
            Log::warning('Missing authentication information for Opening voucher delete', [
                'comp_id' => $compId,
                'location_id' => $locationId,
                'user_id' => $userId,
                'voucher_id' => $id
        ...
        try {
            ...
            if (!$voucher) {
                Log::warning('Opening voucher not found for deletion', [
                    'voucher_id' => $id,
                    'comp_id' => $compId,
                    'location_id' => $locationId
                ]);
            ...
            // Only draft vouchers can be deleted
            if ($voucher->status !== 'Draft') {
                Log::warning('Attempted to delete non-draft Opening voucher', [
                    'voucher_id' => $id,
                    'voucher_number' => $voucher->voucher_number,
            ...
            try {
                $recoveryId = RecoveryService::saveForRecovery(
                    ...
                    $relatedData,
                    $voucherNumber,
                    'Manual deletion from Opening Voucher list'
                );

                Log::info('Opening voucher saved for recovery', [
                    'voucher_id' => $id,
                    'voucher_number' => $voucherNumber,
                    'recovery_id' => $recoveryId
                ]);
            } catch (\Exception $recoveryException) {
                Log::warning('Failed to save Opening voucher for recovery', [
                    'voucher_id' => $id,
                    'voucher_number' => $voucherNumber,
            ...
            Log::info('Deleting Opening voucher entries', [
                'voucher_id' => $id,
                'voucher_number' => $voucherNumber
            ]);
            ...
            Log::info('Opening voucher entries deleted', [
                'voucher_id' => $id,
            ...
            Log::info('Opening voucher attachments deleted', [
                'voucher_id' => $id,
            ...
            Log::info('Opening voucher deleted successfully', [
                'voucher_id' => $id,
                'voucher_number' => $voucherNumber
            ]);
            ...
            // Create audit log for the Opening voucher deletion
            try {
                AuditLogService::logOpeningVoucher('DELETE', $id, [], $oldData);
                Log::info('Audit log created for Opening voucher deletion', [
                    'voucher_id' => $id,
                    'voucher_number' => $voucherNumber
                ]);
            } catch (\Exception $auditException) {
                // Don't fail the main operation if audit logging fails
                Log::warning('Failed to create audit log for Opening voucher deletion', [
                    'voucher_id' => $id,
                    'voucher_number' => $voucherNumber,
                    'error' => $auditException->getMessage()
                ]);
            ...
            return redirect()->route('accounts.Opening-voucher.index')
                ->with('success', "Opening voucher {$voucherNumber} deleted successfully!");
        } catch (\Illuminate\Database\QueryException $e) {
            DB::rollback();

            // Log detailed error for developers
            Log::error('Database error deleting Opening voucher', [
                'error' => $e->getMessage(),
                'sql' => $e->getSql(),
                'bindings' => $e->getBindings(),
                'trace' => $e->getTraceAsString(),
            ...
            // Return user-friendly error message
            return redirect()->back()->with('error', 'There was a problem deleting the Opening voucher. Please try again or contact support.');
        } catch (\Exception $e) {
            DB::rollback();

            // Log detailed error for developers
            Log::error('Error deleting Opening voucher', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'voucher_id' => $id,
                'user_id' => $userId,
            ...
            // Return user-friendly error message
            return redirect()->back()->with('error', 'Something went wrong while deleting the Opening voucher. Please try again or contact support if the problem persists.');
        }
    }
